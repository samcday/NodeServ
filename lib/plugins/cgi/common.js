...
CGIResponseStreamReader.prototype. _processHeaders = function() {
	var headerLines = this._headersRaw.split("\n");
	var headers = {};
...
CGIResponseStreamReader.prototype.parseForHeaders = function(buffer) {
	...
	while(!this.headersParsed)
	{
		...
		this._headersRaw += line + "\n";

		// If there's two newlines in raw headers, it means headers are finished.
		if((this._headersRaw.charAt(this._headersRaw.length - 1) == "\n")
				&& (this._headersRaw.charAt(this._headersRaw.length - 2) == "\n"))
			this._processHeaders();
	}

	this.headersParsed ? this.emit("data", buffer) : (this._headersRaw += buffer.toString("utf8"));
};
