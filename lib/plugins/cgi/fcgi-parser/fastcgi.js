...
Buffer.prototype.unpack = function(format, off) {
	var buffer = this;
	if(off > 0)
		buffer = this.slice(off);
	...
	for(var i = 0; i < format.length; i++) {
		switch(format[i]) {
			...
			{
				// 16 bits.
				result[result.length] = (buffer[ptr++] << 8) | buffer[ptr++]; 
				break;
			}
			case "N":
			{
				// 32 bits.
				result[result.length] = (buffer[ptr++] << 24) | (buffer[ptr++] << 16) | (buffer[ptr++] << 8) | buffer[ptr++]; 
				break;
			}
			case "o":
			{
				// 8 bits.
				result[result.length] = buffer[ptr++];
		...
		switch(record.header.type) {
			...
			case constants.record.FCGI_DATA:
				if(record.header.contentLength > 0) {
					...
					if(_parser.onBody) _parser.onBody(record.body);
				}
				break;
